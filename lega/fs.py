...
class LegaFS(Operations):
    def __init__(self, root, user, domain):
        self._user = user
        self._root = root  # .rstrip('/') # remove trailing /
        self._pending = set()
        self._domain = domain
    ...
    def _send_message(self, path, fh):
        if not self._channel:
            self._connection = get_connection(self._domain)
...
def main():
    ...
    # ....aaand cue music!
    try:
        FUSE(LegaFS(rootdir, user, 'broker'), mountpoint, **options)
