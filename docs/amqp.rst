...
- all messages involved in the ingestion (``inbox``, ``ingest``, ``verified``, ``accession``, ``completed``, ``cancel``, ``error``) 
also **require** a correlation id, i.e. a `36-characters string <https://en.wikipedia.org/wiki/Universally_unique_identifier#Format>`_ (as generated by `uuid_generate <https://linux.die.net/man/3/uuid_generate>`_).
