...
echo "Generating fake EGA users"
...
echo "Generating the EGA configuration files"
...
echo "Generating a CEGA password for the MQ"
...
cat > $ABS_PRIVATE/.env.d/cega_instances <<EOF
LEGA_INSTANCES=SWE1,FIN1
LEGA_SWE1_PASSWORD=${LEGA_SWE1_PASSWORD}
LEGA_FIN1_PASSWORD=${LEGA_FIN1_PASSWORD}
EOF

cat > $ABS_PRIVATE/.env.d/cega.swe1 <<EOF
LEGA_INSTANCE_GREETING=Welcome to Local EGA Sweden @ NBIS
CEGA_ENDPOINT=http://cega_users/user/%s
CEGA_ENDPOINT_USER=SWE1
CEGA_ENDPOINT_PASSWORD=${LEGA_SWE1_PASSWORD}
CEGA_ENDPOINT_RESP_PASSWD=.password_hash
CEGA_ENDPOINT_RESP_PUBKEY=.pubkey
EOF

cat > $ABS_PRIVATE/.env.d/cega.fin1 <<EOF
LEGA_INSTANCE_GREETING=Welcome to Local EGA Finland @ CSC
CEGA_ENDPOINT=http://cega_users/user/%s
CEGA_ENDPOINT_USER=FIN1
CEGA_ENDPOINT_PASSWORD=${LEGA_FIN1_PASSWORD}
CEGA_ENDPOINT_RESP_PASSWD=.password_hash
CEGA_ENDPOINT_RESP_PUBKEY=.pubkey
EOF
